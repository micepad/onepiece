...
pi extension that runs the [basecamp-local-agent-connector](https://github.com/basecamp/basecamp-local-agent-connector) bridge inside pi, but hands each mention off to a background [pi-subagents](https://github.com/micepad) `worker` agent (fresh context, async) instead of doing the work inline. The watcher stays free to take new mentions.
